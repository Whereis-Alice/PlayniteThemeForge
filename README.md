...
> 适用于 Playnite 10.x（Desktop 模式）。当前版本 1.2.0。
...
- **扩展** — 主题声明的必需 / 推荐扩展，未安装的高亮提示；同时列出「被压住的覆盖」，即那些优先级高于所选预设、导致预设看不出效果的单键自定义值，可一键清除。
...
- **被压覆盖检测**：分层规则是「预设 < 主题选项 < 资源覆盖」，所以一个早就忘掉的单键覆盖会让之后换的任何预设都失效，看起来像「预设没用」。插件会把这些键连同取值和来源层列出来，并提供一键交回预设控制的按钮，其余自定义保持不变。
...
| 覆盖压住预设却无从察觉 | 两个上游插件都是「单键覆盖赢」，但没有任何提示；ThemeModifier 的存档还会被主题选项插件完全忽略，用户换预设看不出变化只能靠猜 | 「扩展」页列出所有压住预设的键、取值与所在层，并给出一键清除 |
