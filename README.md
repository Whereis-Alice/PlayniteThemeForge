...
> 适用于 Playnite 10.x（Desktop 模式）。当前版本 1.3.0。
...
- **扩展** — 主题声明的必需 / 推荐扩展，未安装的高亮提示；同时列出需要留意的单键覆盖：压过所选预设的、压住预设调色板颜色的、以及与主题默认值相同因此毫无作用的，每条都标注原因，可一键清除。
...
- **被压覆盖检测**：分层规则是「预设 < 主题选项 < 资源覆盖」，所以一个早就忘掉的单键覆盖会让之后换的任何预设都失效，看起来像「预设没用」。插件不只比对键名：主题里的笔刷普遍写成 `<SolidColorBrush x:Key="GlyphBrush" Color="{DynamicResource GlyphColor}" />`，而配色预设只提供 `GlyphColor`，因此覆盖 `GlyphBrush` 会在键名毫无冲突的情况下把预设废掉。插件会解析主题 xaml 的派生关系并做传递闭包检索，把这类间接冲突一并列出；顺带还会指出与主题默认值完全相同、因此不起任何作用的冗余覆盖。每条结果都标注原因、取值与所在层，并提供一键交回预设控制的按钮，其余自定义保持不变。
...
| 覆盖压住预设却无从察觉 | 两个上游插件都是「单键覆盖赢」，但没有任何提示；ThemeModifier 的存档还会被主题选项插件完全忽略，用户换预设看不出变化只能靠猜 | 「扩展」页解析主题的资源派生关系，列出直接压过预设、间接压住预设调色板颜色、以及完全无作用的冗余覆盖，标注原因并给出一键清除 |
| 迁移会导入全透明画刷 | ThemeModifier 允许颜色保留而不透明度滑到 0，折叠后是 `#00xxxxxx`：画不出任何东西，却因为优先级最高而永久盖掉主题和预设的取值 | 迁移时跳过并在报告里注明 |
